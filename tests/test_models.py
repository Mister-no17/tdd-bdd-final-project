...
class TestProductModel(unittest.TestCase):
    ...
    def test_update_product_no_id(self):
        """It should raise DataValidationError if updating product with no id"""
        product = ProductFactory()
        product.id = None
    ...
    def test_find_by_category_unknown(self):
        """It should cover find_by_category with default UNKNOWN category"""
        ProductFactory(category=Category.UNKNOWN).create()
    ...
    def test_deserialize_missing_key(self):
        ...
        product = Product()
        data = {"description": "desc", "price": 10.0, "available": True, "category": "FOOD"}  # missing name
        with self.assertRaises(DataValidationError):
            product.deserialize(data)

    def test_deserialize_wrong_available_type(self):
        """It should raise DataValidationError for wrong type for available"""
        product = Product()
        data = {"name": "name", "description": "desc", "price": 10.0, "available": "yes", "category": "FOOD"}
        with self.assertRaises(DataValidationError):
            product.deserialize(data)
    ...
    def test_deserialize_invalid_attribute(self):
        ...
        product = Product()
        data = {"name": "name", "description": "desc", "price": 10.0, "available": True, "category": "INVALID"}
        with self.assertRaises(DataValidationError):
            product.deserialize(data)
